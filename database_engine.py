...
def get_coordinates(location_name):
    ...
    return 21.1250, 79.0600 # Fallback 

def get_real_infrastructure(lat, lon, radius_meters=5000):
    """
    Queries the actual OpenStreetMap database for REAL hospitals 
    and police stations near the user's coordinates.
    """
    ...
    try:
        ...
        for element in data['elements']:
            ...
            node_data = {
                "name": name, "lat": e_lat, "lon": e_lon, "dist": dist,
                "phone": tags.get('phone', "Number Not Listed")
            }
...
def generate_local_services(center_lat, center_lon):
    """Combines real infrastructure with our simulated Vanguard/Towing network."""
    real_hospitals, real_police = get_real_infrastructure(center_lat, center_lon)
    
    services = {
        "hospitals": real_hospitals if real_hospitals else [{"name": "Fallback Trauma Center", "lat": center_lat+0.01, "lon": center_lon+0.01, "dist": 2.1, "phone": "108"}],
        "police": real_police if real_police else [{"name": "Fallback Police Station", "lat": center_lat-0.01, "lon": center_lon-0.01, "dist": 1.5, "phone": "100"}],
        "towing": [], "vanguard": []
    }
    
    # Generate the "Network" nodes (Students and Tow Trucks)
    for i in range(3):
