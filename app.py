...
def get_ai_triage(image):
    ...
    try:
        # We removed the manual context box, so the AI relies purely on the image now.
...
if st.session_state.offline_mode:
    st.markdown('<div class="offline-banner">⚠️ 2G EDGE NETWORK / OFFLINE ENGAGED.</div>', unsafe_allow_html=True)

tab1, tab2, tab3 = st.tabs(["🆘 Node 1: Bystander", "🎒 Node 2: Vanguard App", "🏥 Node 3: Command Hub"])

# ==========================================
# TAB 1: THE BYSTANDER (ZERO-CLICK TRIGGER)
# ==========================================
with tab1:
    c1, c2 = st.columns([1.5, 1], gap="large")
    with c1:
        if st.session_state.ecosystem_state == 'idle':
            st.markdown("### 📸 Snap a photo. We handle the rest.")
            st.caption("Auto-locates via network, runs AI triage, and alerts hospitals instantly.")
            
            # The user ONLY sees the camera
        ...
        if st.session_state.ecosystem_state != 'idle' and not st.session_state.offline_mode:
            if st.session_state.ecosystem_state == 'dispatched':
                ...
                <div class='progress-box'>
                    ...
                    <p><b>{st.session_state.active_vanguard['name']}</b> is navigating to you.</p>
...
# ==========================================
# TAB 2 & 3: THE VANGUARD & COMMAND HUB
# ==========================================
with tab2:
